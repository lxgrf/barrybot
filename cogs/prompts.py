...
class Prompts(commands.Cog):
    ...
    async def scene(self, interaction: discord.Interaction, first_character: str, second_character: str, request: str = ""):
        ...
        if str(interaction.guild.id) not in config.guilds:
            ...
            return
        elif not _ai_enabled_server(interaction.guild.id):
    ...
    async def solo(self, interaction: discord.Interaction, character: str, request: str = ""):
        await interaction.response.defer()
        description = ""
        if str(interaction.guild.id) not in config.guilds:
            embed = _server_error(interaction)
